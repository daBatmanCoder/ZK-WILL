...
contract MerkleTreeWithHistory {
    ...
    event LeafInserted(bytes32 indexed leaf, uint256 index, bytes32[] siblingHashes, bytes32 newRoot);
    event SubTree(uint32 index, bytes32 valueOfLeaf);
    ...
    // Declare an array of tuples
    MyTuple[] public myArray;

    // Function to add a tuple to the array
    function addTuple(uint32 _data1, bytes32 _data2) public {
        myArray.push(MyTuple(_data1, _data2));
    }

    // Function to get tuple count
    function getTupleCount() public view returns (uint256) {
        return myArray.length;
    }

    // Function to get tuple data at a specific index
    function getTupleAtIndex(uint256 index) public view returns (uint32, bytes32) {
        require(index < myArray.length, "Index out of bounds");
        MyTuple memory tuple = myArray[index];
