...
The `_depositWill` method implements the `depositWill` method. It has 2 parameters:
- `_commitment` is the commitment of the user.

The `_withdrawWill` method implements the `withdrawWill` method. It has 6 parameters:
...
The commitment and the nullifier hash are generated on the client side by the `generateCommitment` method. (Please check the [VoterRegistration](https://github.com/TheBojda/zktree-vote/blob/main/src/components/VoterRegistration.vue) component in the `zktree-vote` project.) 
