...
async function prepareProofFile() {
	...
	const input = await calculateInputToProof(mimc, levels, commitments, nullifier, secret);

	// calculateInputToProof(mimc,levels,commitments, nullifier, secret);
	const commitment = calculateHash(mimc, nullifier, secret);
	const { root, pathElements, pathIndices } = calculateMerkleRootAndPath(mimc, levels, commitments,commitment );
