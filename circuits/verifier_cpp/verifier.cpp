...
void CommitmentHasher_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void CommitmentHasher_3_run(uint ctx_index,Circom_CalcWit* ctx);
void DualMux_4_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void DualMux_4_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 38307;}

uint get_number_of_components() {return 108;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 28463;}
...
uint sub_component_aux;
uint index_multiple_eq;
Fr_sub(&expaux[3],&circuitConstants[3],&signalValues[mySignalStart + 4]); // line circom 26
Fr_mul(&expaux[1],&signalValues[mySignalStart + 4],&expaux[3]); // line circom 26
...
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
Fr_sub(&expaux[2],&signalValues[mySignalStart + 3],&signalValues[mySignalStart + 2]); // line circom 27
Fr_mul(&expaux[1],&expaux[2],&signalValues[mySignalStart + 4]); // line circom 27
Fr_add(&expaux[0],&expaux[1],&signalValues[mySignalStart + 2]); // line circom 27
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &signalValues[mySignalStart + 1];
// load src
Fr_sub(&expaux[2],&signalValues[mySignalStart + 2],&signalValues[mySignalStart + 3]); // line circom 28
Fr_mul(&expaux[1],&expaux[2],&signalValues[mySignalStart + 4]); // line circom 28
Fr_add(&expaux[0],&expaux[1],&signalValues[mySignalStart + 3]); // line circom 28
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 0; i++){
...
DualMux_4_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 5 ;
aux_cmp_num += 1;
}
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[41]); // line circom 42
while(Fr_isTrue(&expaux[0])){
Fr_eq(&expaux[0],&lvar[1],&circuitConstants[1]); // line circom 44
if(Fr_isTrue(&expaux[0])){
{
...
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
DualMux_4_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 2];
// load src
Fr_sub(&expaux[0],&lvar[1],&circuitConstants[3]); // line circom 44
// end load src
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "commitmentHasher";
CommitmentHasher_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 2660 ;
aux_cmp_num += 6;
}
...
ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1;
assert(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter));
CommitmentHasher_3_run(mySubcomponents[cmp_index_ref],ctx);
}
{
uint cmp_index_ref = 1;
{
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[41]); // line circom 44
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 1;
{
