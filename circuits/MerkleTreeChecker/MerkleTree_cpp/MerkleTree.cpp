...
uint get_total_signal_no() {return 1783;}

uint get_number_of_components() {return 6;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 1326;}
...
Fr_sub(&expaux[3],&circuitConstants[0],&signalValues[mySignalStart + 4]); // line circom 26
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "selectors";
...
uint csoffset = mySignalStart+4;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "hashers";
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[0]); // line circom 42
while(Fr_isTrue(&expaux[0])){
if(Fr_isTrue(&circuitConstants[0])){
{
uint cmp_index_ref = 0;
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 2];
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 2];
// load src
Fr_sub(&expaux[0],&lvar[1],&circuitConstants[0]); // line circom 44
// end load src
